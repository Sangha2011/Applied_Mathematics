...
for iter = 1:numIter
 ...
 if minDist < globalMin && mod(iter,100)==0
    globalMin = minDist;
    rte = routes(idx,:);
    rte = rte([1:n 1]);
    xx=xy(rte,1);
    yy=xy(rte,2);
    if dims < 3 
       plot(xx,yy,'ro','linewidth',1.5);hold on;
       plot(xx,yy,'b-','linewidth',1.5);hold off;
     else 
       zz=xy(rte,3);
       plot3(xx,yy,zz,'ro','linewidth',1.5);hold on;
       plot3(xx,yy,zz,'b-','linewidth',1.5);hold off;
       box on
     end               
...
figure(2); clf; plot(distHistory,'o-')
